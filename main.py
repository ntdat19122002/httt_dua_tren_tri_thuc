...
def first_question(list_symptom_of_person, person):
    ...
    while (1):
        ...
        if (len(list_symptom_of_person) == 0):
            print(
                f'-->Chatbot: {person.name} có triệu trứng nào ở dưới đây không (Nhập số thứ tự của triệu chứng để chọn. Có thể lựa chọn nhiều)')
        else:
            print(
                f'-->Chatbot: {person.name} có triệu trứng nào nữa ở dưới đây không (Nhập số thứ tự của triệu chứng để chọn. Có thể lựa chọn nhiều)')
...
def second_question(list_symptom_of_person, person):
    ...
    while (1):
        ...
        if (check in list_symptom_of_person):
            print(
                f'-->Chatbot: {person.name} đang có triệu chứng ĐAU BỤNG- một trong số các triệu chứng của các bệnh về dạ dày.\n Để có chuẩn đoán chính xác, hãy cho tôi biết chi tiết thêm về vị trí đau')
...
def forth_question_before_forward_inference(list_symptom_of_person, person):
    ...
    initTree = TreeForFC('S09', TreeForFC('S14', TreeForFC('S16', TreeForFC('S30'), TreeForFC('S20')),
                                          TreeForFC('S22', TreeForFC('S31'), TreeForFC('S21'))),
                         TreeForFC('S10', TreeForFC('S17', TreeForFC('S11'), TreeForFC('S26')),
...
def forward_chaining(rule, fact, goal, file_name, person):
    ...
    print(
        f'-->Chatbot: Trên đây là chuẩn đoán sơ bộ của chúng tôi. Tiếp theo, chúng tôi sẽ hỏi {person.name} một số câu hỏi để đưa ra kết quả chính xác.',
        end=" ")
