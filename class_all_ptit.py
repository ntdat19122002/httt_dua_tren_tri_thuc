...
class ConvertData:
    """
    Truy vấn và xử lý dữ liệu (Constructer)
    """
    ...
    def convertbenh(self):
        ...
        """
        # .cursor kết nối đến database
        dbbenh = mydb.cursor()
        # Lấy ra tất cả các bệnh trong database cumptit
        dbbenh.execute("SELECT * FROM cumptit.benh;")
    ...
    def converttrieuchung(self):
        ...
        dbtrieuchung.execute("SELECT * FROM cumptit.trieuchung;")
    ...
    def getfc(self):
        ...
        for i in range(len(fc)):
            # s giúp lấy ra id triệu chứng
            s.append(fc[i][2])
            # d giúp lấy ra id bệnh
            d.append(fc[i][3])
    ...
    def getbc(self):
        ...
        dbbc = mydb.cursor()
        dbbc.execute(
            "select idsuydien, luat.idluat, idtrieuchung, idbenh, trangthai from suydien, luat where suydien.idluat=luat.idluat and trangThai='0' order by idbenh")
        fc = dbbc.fetchall()
        rule = []
        s = []
        d = []
        for i in range(len(fc)):
            # rule lấy list id luật
            rule.append(fc[i][1])
            # s lấy id triệu chứng
            s.append(fc[i][2])
            # d lấy id bệnh
            d.append(fc[i][3])
        # print(rule)
    ...
    def gettrieuchung(self):
        ...
        """
        dbtrieuchung = mydb.cursor()
        dbtrieuchung.execute("SELECT * FROM cumptit.suydien order by idbenh")
